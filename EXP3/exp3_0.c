...
int main(void)
{
    ...
    while (1)
    {				
        uart_receive_char = UARTCharGet(UART2_BASE);   // 从UART读入一个字符
        ...
        UARTCharPut(UART2_BASE, uart_receive_char);  // 发送转换好的字符                 
...
// 函数功能：UART初始化。使能UART2，设置PD4,PD5为UART2 RX,TX引脚；
//          设置波特率及帧格式。
// 函数参数：无
// 函数返回值：无
//
...
{
    // 引脚配置
    SysCtlPeripheralEnable(SYSCTL_PERIPH_UART2);        // 使能UART2模块   
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOD);        // 使能端口 A
    while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOD));	// 等待端口 A准备完毕 

    GPIOPinConfigure(GPIO_PD4_U2RX);				  // 设置PD4为UART2 RX引脚
    GPIOPinConfigure(GPIO_PD5_U2TX);    			  // 设置PD5为UART2 TX引脚        

	// 设置端口 A的第0,1位（PD4,PD5）为UART引脚
    ...
    //UARTStringPut(UART2_BASE, (const char *)"\r\nHello, 2A!\r\n");	
}
