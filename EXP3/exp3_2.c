...
bool match4(uint8_t *str);
bool match5(uint8_t *str);
...
int main(void)
{
    ...
    TM1638_Init();	          // 初始化TM1638
     ...
     UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)"READY\r\n");
    ...
    {				
      ...
      {
				//uint8_t o1[3] = {0};
				//o1[0] = '0'+uart_receive_len;
				//o1[1] = '\n';
				//UARTStringPutNOBlocking(UART0_BASE, o1);

        cmd_state = 0;
        uart_receive_len = 0;  
				//UARTStringPutNOBlocking(UART0_BASE, uart_receive_buffer);				
        // 根据实际要求添加处理代码
				
				if(match1(uart_receive_buffer)){//GET command
					//UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)time_now);
						UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)"CLASS2245\r");
				} else if(match2(uart_receive_buffer)){//SET command
					uint8_t h = (uart_receive_buffer[6]-'0') * 10 + (uart_receive_buffer[7]-'0');
					uint8_t c = (uart_receive_buffer[9]-'0') * 10 + (uart_receive_buffer[10]-'0');
					uint8_t s = (uart_receive_buffer[12]-'0') * 10 + (uart_receive_buffer[13]-'0');
					if(h > 23 || c > 59|| s > 59){
						...
						time_s = s;
						time_str();
						//UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)time_now); //AT+SET不输出到终端
					}
				} else if(match3(uart_receive_buffer)){//INC command
					uint8_t h = (uart_receive_buffer[6]-'0') * 10 + (uart_receive_buffer[7]-'0');
					uint8_t c = (uart_receive_buffer[9]-'0') * 10 + (uart_receive_buffer[10]-'0');
					uint8_t s = (uart_receive_buffer[12]-'0') * 10 + (uart_receive_buffer[13]-'0');
					if(h > 23 || c > 59|| s > 59){
						UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)"Error Command!\r");
					} else if(match4(uart_receive_buffer)){//CLASS
						UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)"CLASS2245\r");
					} else if(match5(uart_receive_buffer)){//STUDENTCODE
						UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)"CODE522031910255\r");
					} else {
						add_time(h, c, s);
						time_str();
						UARTStringPutNOBlocking(UART0_BASE, (uint8_t*)time_now);
...
bool match4(uint8_t *str){//CLASS
	uint8_t i = 0;
	for(; i < 8; ++i){
		if(str[i] != AT_CLASS[i]) return false;
	}
	if(str[9] == '\0') return true;
	return false;
}

bool match5(uint8_t *str){//STUDENTCODE
	uint8_t i = 0;
	for(; i < 14; ++i){
		if(str[i] != AT_STUDENTCODE[i]) return false;
	}
