...
//
//*****************************************************************************
#define SYSTICK_FREQUENCY		4000		// SysTick频率为50Hz，即循环定时周期20ms

#define FREQUENCY_MIN     300           // 有效频率最小值，根据实际需要设置
#define FREQUENCY_MAX     4000         // 有效频率最大值，根据实际需要设置

#define V_T100ms    400              // 0.1s软件定时器溢出值，5个20ms
#define V_T400ms	1600             // 400ms软件定时器溢出值，20个20ms
#define V_T500ms    2000           // 0.5s软件定时器溢出值，25个20ms
#define V_T1500ms   6000            // 1.5s软件定时器溢出值，设为测温间隔
#define V_T2s       8000          // 2s软件定时器溢出值，100个20ms
#define V_T5s       20000          // 5s软件定时器溢出值
#define V_T10s      40000         // 10s软件定时器溢出值
//*****************************************************************************
//
// 函数原型声明
//
...
void UARTStringPut(uint32_t ui32Base,const char *cMessage);// 向UART发送字符串。发送到缓冲区
void UARTStringGet(uint32_t ui32Base, char* s);//读取字符串
void UART6_Handler(void);//中断处理程序
...
uint8_t clock2s_flag = 1;  //初始2s标志位为1
...
uint16_t voltage = 0; //保存电压值
uint8_t* v_passin[4] = {0}; //测量电压显示
uint16_t tempre = 0;//保存温度值
uint8_t* t_passin[3] = {0}; //测量温度显示
uint8_t freq[4] = {1, 0, 1, 0};
uint8_t freq_temp[4] = {0};    // 用于保存频率参数，若不合法则返回修改前参数
uint8_t* f_passin[4] = {0}; //显示频率

// 定义函数指针数组便于切换模式
void (*func[3][5])(uint8_t* [], uint8_t* [], uint8_t* []) = {0};

// AIN2(PE1)  ADC采样值[0-4095]
uint32_t ui32ADC0Value; //PE1为输入引脚

// AIN2电压值(单位为0.01V) [0.00-3.30]
uint32_t ui32ADC0Voltage;//电压值与ADC采样值线性对应
...
int main(void)
{
    ...
    passFreq(freq);
		...
		UARTStringPut(UART6_BASE, "AT+VOL=15");
    ...
    while (1)
    {
            ...
            if (clock400ms_flag == 1)             // 检查400ms秒定时是否到
            {
                ...
                ui32ADC0Value = ADC_Sample();   // 采样，一次采一个，400ms采样一次
    ...
    //            digit[7] = ui32ADC0Value % 10;           // 显示ADC采样值个位数
                ...
                ui32ADC0Voltage = ui32ADC0Value * 3300 / 4095; //采样的值线性对应电压
    ...
    //            digit[3] = ui32ADC0Voltage % 10;         // 显示电压值百分位数
                ...
                if (lcd_act == 0 && lcd_subact == 0) if_refresh = true; //处于模式0则刷新，为了更新频率值
            }
    ...
    //            }
            ...
            {
                if (if_count_2s && clock2s_flag){ //处于警示界面，2s计时器触发，两秒后回到ACT1
                    lcd_act = 1;
                    lcd_subact = 0;
                    if_count_2s = false;
                }
...
void SysTick_Handler(void)       // 定时周期为20ms
{
    ...
    if(key_code_pre != 1 && key_code == 1){
        system_state = !system_state;
    }

    if(key_code_pre != 3 && key_code == 3){
        non_stop = !non_stop;
    }
    ...
    signal = GPIOPinRead(GPIO_PORTL_BASE, GPIO_PIN_4);
...
void UARTInit(void)
{
    ...
    GPIOPinConfigure(GPIO_PP0_U6RX);				  // 设置PP0为UART6 RX引脚，RX为接收引脚
    GPIOPinConfigure(GPIO_PP1_U6TX);    			  // 设置PP1为UART6 TX引脚，TX为发送引脚
    ...
    UARTIntEnable(UART6_BASE, UART_INT_RX | UART_INT_RT);// 使能 UART6 RX,RT 中断。RX为接收中断，RT为接收超时中断
//BASE表示段基址地址。用这个地址来指向对应模块。用来发射或接受等等
    // 引脚配置
...
void UART6_Handler(void) //UART中断服务程序
{
    ...
    UARTStringGet(UART6_BASE, str); //读取UART6发送的字符
    UARTStringPut(UART0_BASE, str); //发送到缓冲区等待UART0接收
}
...
void passFreq(uint8_t* f){ //根据频率值设置接收频率
...
//
//*******************************************************************************************************
void PWMInit(uint32_t ui32Freq_Hz) //发生方波
{
