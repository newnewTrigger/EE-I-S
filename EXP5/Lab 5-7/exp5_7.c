...
#define V_T1500ms   3              // 1.5s软件定时器溢出值，设为测温间隔  改为60ms
...
int main(void)
{
    ...
    //            digit[3] = ui32ADC0Voltage % 10;         // 显示电压值百分位数
            ...
            // ****************** 计时测量频率 *******************clock1500ms_flag == 1
            if (start == false){ // 检查1.5秒定时是否到，启动Timer0A
    ...
    //            }
                ...
                if(play_record == false){
                    ...
                    PWMStart(record_melody[ii]);
                    ++ii;
                    if(ii >= 400){
                        ii = 0;
                        PWMStop();
                        SysCtlDelay((ui32SysClock * 2 / 3) * 1);
                        play_record = false;
                    }
                }
...
void Timer1Init(void)
{
    ...
    TimerLoadSet(TIMER1_BASE, TIMER_A, (ui32SysClock / 10) - 1);  // TIMER1A装载计数值0.1s
}
